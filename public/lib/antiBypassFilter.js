...
function inspectAndSanitizeMessage(messageText) {
  ...
  // 1. Détection des mots-clés d'évasion
  for (const keywordRegex of EXTERNAL_REDIRECT_KEYWORDS) {
    if (keywordRegex.test(sanitized)) {
      violations.push(`Canal externe détecté : ${keywordRegex.source}`);
      sanitized = sanitized.replace(keywordRegex, '[Information de contact masquée par sécurité Relais]');
    }
  }

  // 2. Détection des numéros de téléphone sur texte brut
  for (const pattern of PHONE_PATTERNS) {
    if (pattern.test(sanitized)) {
      violations.push(`Numéro ou suite de chiffres détectée`);
      sanitized = sanitized.replace(pattern, '[Numéro masqué par sécurité Relais]');
    }
  }

  // 3. Détection des numéros dissimulés en lettres
  const normalized = normalizeText(messageText);
  for (const pattern of PHONE_PATTERNS) {
    if (pattern.test(normalized)) {
      if (!violations.length) {
        violations.push("Numéro dissimulé en lettres détecté");
        sanitized = '[Message masqué : les coordonnées directes sont interdites sur Relais]';
      }
    }
  }
